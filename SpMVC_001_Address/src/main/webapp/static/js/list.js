/*
  ...
  리스트 화면에서 항목을 클릭했을때, Detail 주소정보를 보여주는
  화면으로 화면전환을 하려고 한다.

  사용자가 List 의 어딘가를 클릭하면 그 클릭한 event 를 설정 해야 한다.
  List 에서 최종적으로 클릭되는 요소는 Table > Tr > Td 에서 TD 요소가 된다.
  현재 화면에서 10명의 주소가 보이고 있는데, 이 화면의 TD 요소에
  이벤트 핸들링을 수행 하려면 40개의 이벤트를 설정해야 한다.

  이러한 방식은 코드 작성은 그리 많이 어렵지 않으나, 실제 작동되는 과정에서
  많은 문제를 일으킬수 있다.

  JS 에서 querySelectorAll() 사용하여 만든 List 에 event 핸들링을
  설정하는 것은 매우 불합리한 코드가 될수 있다.

  이럴때는 querySelectorAll() 을 사용해야하는 tag 들을 감싸고 있는
  ...
  그 tag 가 클릭 되었을때 실제 필요한 코드를 작성한다.
*/
document.addEventListener("DOMContentLoaded", () => {
  ...
  // 실제로는  table > tr > td 을 클릭 되는것을 감지하여 코드 실행
