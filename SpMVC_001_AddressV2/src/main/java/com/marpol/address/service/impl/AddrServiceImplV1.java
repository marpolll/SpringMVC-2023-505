...
import com.marpol.address.dao.HobbyDao;
import com.marpol.address.dao.ScoreDao;
import com.marpol.address.models.AddrDto;
import com.marpol.address.models.HobbyByAidVO;
import com.marpol.address.service.AddrService;

@Service
public class AddrServiceImplV1 implements AddrService {

	/*
	 * 의존성 주입(Dependency Inject: DI) 객체를 선언하고, 클래스의 생성자를 통해서 객체를 초기화 하는 java 의 전통적인
	 * 객체 생성 코드와 대비되는 Spring Framework Project 의 독특한 기법 객체를 선언만 하고, 의존성 주입 절차를 수행하면
	...
	/*
	 * 필드(클래스영역)에 객체를 선언하고 @Autowired 를 부착하는 방법 Spring Project 의 전통적인 코딩에서 가장 많이
	 * 사용하던 방법 이 방법이 메모리 관리 측면에서 문제가 있다는 보고가 있으면서 사용이 점차 줄어들고 있다. 이 방법은 메모리 누수를 줄이기
	 * 위하여 객체를 private 으로 선언하는것을 권장하는 방법이다. 객체를 private 으로 선언하면 상속(extends)을 했을때 다시
	 * 객체를 선언해야 하는 문제가 발생한다.
	 * 
	 * 참고로 메모리 누수 정도 public > protected > private 순으로 작아진다.
	 */
	@Autowired
	protected ByuerDao byuerDao;

	// 생성자 의존성 주입(Constructor DI (Dependency Inject))
	/*
	 * 생성자를 통해서 주입될 객체는 final 로 선언을 한다. 이 방식의 특징은 한번 주입이 되면, 코드가 작동되는 중에 또 다시 주입을 할수
	 * 없다. 객체 보호가 완전하다. 객체의 순환참조가 없다. 이 방식은 메모리 누수가 없고, 객체가 보호되며, 순환참조에 의한 코드의
	 * exception 을 막는다.
	 * 
	 * @Autowired 를 붙이지 않아도 된다.
	 * 
	 * 이 방식을 사용하려면 반드시 생성자 메서드가 있어야 한다. 코드가 다소 번잡해 진다. 실수로 생성자 메소드를 만들지 않았을 경우
	 ...
	 * 
	 * 누군가 실수로, 임의로 객체를 변경할수 있다 method 가 public 으로 완전 개방된 상태이기 때문에
	 */
